                ...
                default:
                    {
                        ...
                        if (game == null)
                            break;
                        if (game.SpeakerId == 0 || game.SpeakerId == message.From.Id || game.IsPaused)
                            break;
                        else
                        {
...
void GiveUserStats(int gameId)
{
    ...
    foreach (BUser user in game.Users)
    {
        ...
        user.SpecialCards.Add(listSpecialCard[i]);
    ...
    for (int i = 0; i < game.MaxVotesUsers.Count; i++)
    {
        if (game.MaxVotesUsers[i].TelegramId == game.SpeakerId)
        {
            Message sentMessage = await botClient.SendTextMessageAsync(
                  ...
                  cancellationToken: cancellationToken);
            if (game.MaxVotesUsers[i] == game.MaxVotesUsers.Last())
            {
                sentMessage = await botClient.SendTextMessageAsync(
                    chatId: game.GroupId,
...
async void CharactersMenu(BUser user, int index, ITelegramBotClient botClient, CancellationToken cancellationToken)
{
    ...
    if (u.LuggagesOpened)
    {
        if (u.Luggages.Count == 0)
        {
            text += "Багаж | Відсутній\n";
        }
        else
        {
...
async void GameInfoMenu(BUser user, ITelegramBotClient botClient, CancellationToken cancellationToken)
{
    var chatId = user.TelegramId;
    List<InlineKeyboardButton[]> keyboardButtons = new();
    string text = $"Катастрофа:\n {user.BGame.Catastrophe.Name}\n\n Бункер:\n";
    for (int i = 0; i < user.BGame.Status; i++)
    {
        ...
        text += $"\"{user.BGame.BunkerInfos[i].Name}\"\n\n";
    }
    Message sentMessage = await botClient.SendTextMessageAsync(
                  chatId: user.TelegramId,
                  text: text,
        ...
        case "Profession":
            {
                ...
                Message sentMessage = await botClient.SendTextMessageAsync(
                chatId: user.BGame.GroupId,
                text: $"{user.Name} розкриває свою професію: {user.Profession.Name}",
                cancellationToken: cancellationToken);
                db.SaveChanges();
                MainMenu(user, botClient, cancellationToken);
        ...
        case "Biology":
            {
                ...
                Message sentMessage = await botClient.SendTextMessageAsync(
                chatId: user.BGame.GroupId,
                text: $"{user.Name} розкриває свою біологію: {user.Biology.Name}",
                cancellationToken: cancellationToken);
                db.SaveChanges();
                MainMenu(user, botClient, cancellationToken);
            }
        ...
        case "Hobby":
            {
                ...
                Message sentMessage = await botClient.SendTextMessageAsync(
                chatId: user.BGame.GroupId,
                text: $"{user.Name} розкриває своє хоббі: {user.Hobby.Name}",
                cancellationToken: cancellationToken);
                db.SaveChanges();
                MainMenu(user, botClient, cancellationToken);
            }
        ...
        case "Health":
            {
                ...
                Message sentMessage = await botClient.SendTextMessageAsync(
                chatId: user.BGame.GroupId,
                text: $"{user.Name} розкриває свій стан здоров'я: {user.HealthCondition.Name}",
                cancellationToken: cancellationToken);
                db.SaveChanges();
                MainMenu(user, botClient, cancellationToken);
            }
        ...
        case "AddInfo":
            {
                ...
                Message sentMessage = await botClient.SendTextMessageAsync(
                chatId: user.BGame.GroupId,
                text: $"{user.Name} розкриває факт про себе: {user.AdditionalInfo.Name}",
                cancellationToken: cancellationToken);
                db.SaveChanges();
                MainMenu(user, botClient, cancellationToken);
            }
        ...
        case "Luggage":
            {
                ...
                Message sentMessage = await botClient.SendTextMessageAsync(
                chatId: user.BGame.GroupId,
                text: $"{user.Name} розкриває своій багаж: {user.Luggages[0].Name}",
                cancellationToken: cancellationToken);
                db.SaveChanges();
                MainMenu(user, botClient, cancellationToken);
            }
        ...
        case "FirstSCard":
            {
                ...
                Message sentMessage = await botClient.SendTextMessageAsync(
                chatId: user.BGame.GroupId,
                text: $"{user.Name} розкриває свою спеціальну карту: {user.SpecialCards[0].Name}",
                cancellationToken: cancellationToken);
                MainMenu(user, botClient, cancellationToken);
                db.SaveChanges();
                PauseGame(user.BGame.GroupId, botClient, cancellationToken);
        ...
        case "SecondSCard":
            {
                ...
                Message sentMessage = await botClient.SendTextMessageAsync(
                chatId: user.BGame.GroupId,
                text: $"{user.Name} розкриває свою ДОДАТКОВУ спеціальну карту: {user.SpecialCards[1].Name}",
                cancellationToken: cancellationToken);
                MainMenu(user, botClient, cancellationToken);
                db.SaveChanges();
                PauseGame(user.BGame.GroupId, botClient, cancellationToken);
...
void GiveNewCard(int userId, string type)
{
    List<Profession> listProfesions = new(db.Professions);
    List<Hobby> listHobbies = new(db.Hobbies);
    List<Luggage> listLuggages = new(db.Luggages);
        ...
        case "Health":
            {
                ...
                while (!uni)
                {
                    uni = true;
                    i = Random.Shared.Next(0, listHealthConditions.Count);
                    Console.WriteLine(listHealthConditions[i].Name);
                    foreach (BUser u in user.BGame.Users)
                    {
...
void SwapCards(int user1Id, int user2Id, string type)
{
...
void StealLuggage(int user1Id, int user2Id)
{
    BUser user1 = db.Users.Find(user1Id);
    BUser user2 = db.Users.Find(user2Id);
...
async void AdminMenu(BUser user, ITelegramBotClient botClient, CancellationToken cancellationToken)
{
    ...
    Message sentMessage = await botClient.SendTextMessageAsync(
                  ...
                  cancellationToken: cancellationToken);
    InlineKeyboardMarkup inlineKeyboard = new(new[]{
                        new[]{InlineKeyboardButton.WithCallbackData("Почати голосування", $"{user.Id}_startVoting")},
                        new[]{InlineKeyboardButton.WithCallbackData("ПАУЗА", $"{user.Id}_pauseGame")},
                        new[]{InlineKeyboardButton.WithCallbackData("ПРИБРАТИ З ПАУЗИ", $"{user.Id}_unpauseGame")},
                        new[]{InlineKeyboardButton.WithCallbackData("Розкрити характеристику гравця",$"{user.Id}.{sentMessage.MessageId}_OpenCardMenu")},
                        new[]{InlineKeyboardButton.WithCallbackData("Змінити Характеристику гравця", $"{user.Id}.{sentMessage.MessageId}_ChangeCardMenu")},
                        new[]{InlineKeyboardButton.WithCallbackData("Обміняти Характеристики двох гравців", $"{user.Id}.{sentMessage.MessageId}_SwapCardsMenu")},
                        new[]{InlineKeyboardButton.WithCallbackData("Перемішати Характеристику відкритих гравців", $"{user.Id}.{sentMessage.MessageId}_ShuffleCardsMenu")},
                        new[]{InlineKeyboardButton.WithCallbackData("Вкрасти багаж", $"{user.Id}.{sentMessage.MessageId}_StealLuggageMenu")},
                        new[]{InlineKeyboardButton.WithCallbackData("Змінити Характеристику Бункера", $"{user.Id}.{sentMessage.MessageId}_BunkerCardMenu")},
                        new[]{InlineKeyboardButton.WithCallbackData("Передати роль ведучого", $"{user.Id}.{sentMessage.MessageId}_NewAdminMenu")},
                        new[]{InlineKeyboardButton.WithCallbackData("ГОЛОВНЕ МЕНЮ", $"{user.Id}.{sentMessage.MessageId}_MainMenu")}
                    });
    await botClient.EditMessageReplyMarkupAsync(chatId, sentMessage.MessageId, inlineKeyboard);
}
